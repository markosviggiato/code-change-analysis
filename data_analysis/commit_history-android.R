...
android_dataset <- read.table("final_dataset_android-read.txt", sep = ',', stringsAsFactors = FALSE, header = TRUE)
...
for (i in 1:length(android_dataset[,1])){
      ...
      write.table(commit_info, paste0("commit_history/android/", strsplit(fullname, "/")[[1]][2], ".txt"), row.names = FALSE, sep = ',')
      write.table(commit_error, paste0("commit_history/android/", strsplit(fullname, "/")[[1]][2], "_ERROR.txt"), row.names = FALSE, sep = ',')
}
write.table(repos_info, "commit_history/repos_info_android.txt", row.names = FALSE, sep = ',')
